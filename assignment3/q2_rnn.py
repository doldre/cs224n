...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence_hat = [sentence[i] if i < len(sentence) else zero_vector for i in range(max_length)]
        labels_hat = [labels[i] if i < len(labels) else zero_label for i in range(max_length)]
        mask = [True if i < len(labels) else False for i in range(max_length)]
        ret.append((sentence_hat, labels_hat, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32, shape=(None, self.max_length, self.config.n_features), name="input"
        )
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(features, 
