...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(
            tf.float32, [self.config.batch_size, self.config.n_classes]
        )
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        self.W = tf.Variable(
            tf.zeros([self.config.n_features, self.config.n_classes]), name="weight"
        )
        self.b = tf.Variable(
            tf.zeros([self.config.n_classes]), name="biases"
        )
